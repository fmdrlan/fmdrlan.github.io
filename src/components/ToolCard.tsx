...
const CARD_CLASS =
  'group flex items-center gap-4 rounded-[10px] border border-border bg-bg2 p-4 no-underline transition-[border-color,background] duration-150 hover:border-accent/50 hover:bg-accent/[0.04]'
...
export function ToolCard({
  ...
  const inner = <CardInner Icon={Icon} name={name} desc={desc} badge={badge} />
  ...
  return (
    <a href={href} {...targetProps} className={CARD_CLASS}>
      {inner}
    </a>
  )
}
